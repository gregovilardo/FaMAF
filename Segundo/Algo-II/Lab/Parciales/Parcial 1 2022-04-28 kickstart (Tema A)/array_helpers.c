...
unsigned int passengers_amount_in_airport (LayoverTable a, unsigned int h) {
  /* COMPLETAR */
  unsigned int waiting_ps = 0;
  for (unsigned int i = 0; i <= h; i++){
    if (i == h) {
     waiting_ps += a[i][0].passengers_amount;
    } else {
     waiting_ps += a[i][0].passengers_amount - a[i][1].passengers_amount; 
    }
  }
  return waiting_ps;
}
...
void array_from_file(LayoverTable array, const char *filepath) {
  ...
  while (!feof(file)) {
    ...
    Flight flight_arrival = flight_from_file(file, code) /* completar... */;
    Flight flight_departure = flight_from_file(file, code) /* completar... */;
